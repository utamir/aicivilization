...
const fossilBase = new WeakMap();
const activityBase = new WeakMap();
function initialFossilTWh(w) {
...
function buildRatesGWperYear(w) {
    ...
    for (const p of w.projects) {
        ...
        const k = p.kind;
        if (k === 'solar' || k === 'wind' || k === 'geothermal' || k === 'bioenergy' || k === 'ocean_energy' || k === 'storage' || k === 'hydrogen_hub' || k === 'grid' || k === 'nuclear' || k === 'fusion' || k === 'power_satellite')
            clean += perYear * (k === 'storage' || k === 'hydrogen_hub' ? 3 : k === 'grid' ? 0.35 : 1);
        else if (k === 'datacenter' || k === 'fab')
            compute += perYear * (k === 'fab' ? 4 : 1);
        else if (k === 'housing' || k === 'arcology' || k === 'underground_habitat' || k === 'floating_district' || k === 'subsea_habitat')
            housing += perYear * 40;
    }
    return { clean, compute, housing };
}
export function stepResources(w) {
    const r = w.resources;
    ...
    if (remaining <= 0.5)
        fossilTarget = 12; // effectively unavailable
    ...
    if (r.mineralReserves + tailMin <= 0.5)
        minTarget = Math.max(minTarget, 6);
...
function recyclingProjectBonus(w) {
...
export function materialCapexMultiplier(w, civId, kind) {
...
export function fossilDispatchFactor(w) {
