...
import { CAPACITY_FACTOR } from './world.js';
import { CALIBRATION } from '../data/calibration.js';
import { candidatesFor, nextCandidate, followOnCandidate } from '../data/inventions.js';
import { nextFloat, gaussian, chance } from '../rng.js';
import { taskHorizonHrs, autonomyIndex, clamp01, clamp, lerp } from './formulas.js';
import { stepResources, materialCapexMultiplier } from './resources.js';
import { stepFrontier, reconcileFrontierPopulation, planFrontierProjects, agiToolingBoost, physicalPopulation, applyFrontierCompletion, frontierProjectSpec } from './frontier.js';
import { fossilDispatchFactor } from './resources.js';
import { stepDecisions } from './decisions.js';
import { stepSocietyDynamics } from './society.js';
import { reconcile, stepCohorts } from './demography.js';
import { stepLand, planLandProjects, landProjectSpec, applyLandCompletion, landRoomForHousing, housingLandCapexMultiplier } from './land.js';
...
export function yearOf(w) { return w.startYear + w.tMonths / 12; }
/**
 ...
 */
export function energyHardship(c) {
...
export function addEvent(w, e) {
    ...
    if (w.flags['world-extinct'] && e.title !== 'The last population disappears')
        return ev;
    w.chronicle.push(ev);
    return ev;
}
function activeEffortMult(w, techId) {
    let m = 1;
    for (const iv of w.activeInterventions)
        for (const ef of iv.effects)
            if (ef.kind === 'effort' && ef.techId === techId)
                m *= ef.mult;
    return m;
}
function civPolicy(w, civId, policy) {
    let s = 0;
    for (const iv of w.activeInterventions)
        for (const ef of iv.effects)
...
function activeGlobalMult(w, param) {
    let m = 1;
    for (const iv of w.activeInterventions)
        for (const ef of iv.effects)
            if (ef.kind === 'global' && ef.param === param)
                m *= ef.mult;
    return m;
}
export { taskHorizonHrs };
export const techTickCache = {};
function dependencyReadiness(w, techId) {
...
function stepTechnology(w) {
    ...
    for (const id of TECH_IDS) {
        ...
        const t = w.techs[id];
        const info = { growthRate: 0, accelerators: [], constraints: [] };
        ...
        if (depReady < 0.65)
            info.constraints.push(`dependencies immature (${(depReady * 100) | 0}%)`);
        ...
        if (w.frontier.asi)
            info.accelerators.push('superintelligent research systems');
        else if (w.frontier.agi)
            info.accelerators.push('general-intelligence research systems');
        // domain speed knobs
        let domainMult = 1;
        if (id === 'ai_models' || id === 'ai_agents')
            domainMult *= w.params.aiProgressMult * aiFeedbackMult;
        if (id === 'robotics_ind' || id === 'robotics_gp')
            domainMult *= w.params.roboticsMult;
        if (id === 'solar_pv' || id === 'wind_power' || id === 'storage_batt')
            domainMult *= 0.7 + 0.3 * w.params.cleanLearningMult;
        if (id === 'fusion_power')
            domainMult *= w.params.fusionMult;
        if (id === 'longevity_bio')
            domainMult *= w.params.longevityMult;
        if (id === 'space_systems')
            domainMult *= w.params.spaceMult;
        if ((id === 'ai_models' || id === 'ai_agents') && aiFeedbackMult > 1.15)
            info.accelerators.push('AI improving AI research');
        ...
        if (id === 'ai_models' || id === 'ai_agents' || id === 'compute_accel') {
            ...
            let sup = 0, dem = 0;
            for (const c of w.civs) {
                sup += c.compute.supplyFlops;
                dem += c.compute.demandFlops;
            }
        ...
        if (id === 'compute_accel' || id === 'semiconductor_fab' || id === 'ai_models' || id === 'ai_agents') {
            ...
            if (meanPrice > 1.18) {
                constraintGate *= clamp(1.9 - meanPrice * 0.62, 0.3, 1);
                info.constraints.push('electricity cost');
            }
        }
        if (id === 'semiconductor_fab' || id === 'nuclear_power' || id === 'grid_transmission') {
            ...
            if (physGate < 0.85)
                info.constraints.push('physical build-out lags designs');
        ...
        if (t.cap > 2)
            info.constraints.push('increasing research difficulty');
        ...
        for (const c of w.civs) {
            ...
            if (d.energyPerAdoption > 0.2) {
                ...
                if (infraGate < 0.8)
                    info.constraints.push(`${c.name}: electricity margin`);
            }
            if (id === 'transport_ev')
                infraGate *= clamp(0.4 + w.techs.grid_transmission.adoption[c.id].penetration, 0.3, 1);
        ...
        const bottlenecks = [];
        if (headroom < 0.25)
            bottlenecks.push('paradigm asymptote');
        if (t.depReady < 0.6)
            bottlenecks.push('immature dependencies');
        if (difficulty > 3)
            bottlenecks.push('research difficulty');
        if (constraintGate < 0.85)
            bottlenecks.push('compute/energy constraint');
        if (t.cost > 1.5)
            bottlenecks.push('unit cost');
        if (t.reliability < 0.55)
            bottlenecks.push('reliability');
        if (t.mfg < 0.8)
            bottlenecks.push('manufacturing capacity');
...
const LAND_KINDS = ['land_reclamation', 'floating_district', 'seabed_mining', 'offshore_energy', 'subsea_habitat', 'aerial_platform', 'arcology', 'underground_habitat'];
const FRONTIER_KINDS = ['fusion', 'recycling', 'spaceport', 'asteroid_mining', 'orbital_habitat', 'mars_colony', 'interstellar_ark', 'space_elevator', 'power_satellite'];
function projectCapex(kind, capacity) {
    if (kind === 'solar')
        return capacity * CALIBRATION.generation.solar.capexPerGW;
    if (kind === 'wind')
        return capacity * CALIBRATION.generation.wind.capexPerGW;
    if (kind === 'nuclear')
        return capacity * CALIBRATION.generation.nuclear.capexPerGW;
    if (kind === 'geothermal')
        return capacity * CALIBRATION.generation.geothermal.capexPerGW;
    if (kind === 'bioenergy')
        return capacity * CALIBRATION.generation.bioenergy.capexPerGW;
    if (kind === 'ocean_energy')
        return capacity * CALIBRATION.generation.ocean.capexPerGW;
    if (kind === 'hydrogen_hub')
        return capacity * 0.085;
    if (kind === 'fossil')
        return capacity * CALIBRATION.generation.fossil.capexPerGW;
    if (kind === 'grid')
        return capacity * 0.014; // normalized from multi-year transmission capex; derived gameplay scale
    if (kind === 'datacenter')
        return capacity * CALIBRATION.datacenter.capexPerGW;
    if (kind === 'housing')
        return capacity * CALIBRATION.housing.capexPerStockPoint;
    if (kind === 'fab')
        return capacity * CALIBRATION.fab.capexPerUnit;
    if (kind === 'storage')
        return capacity * 0.035;
    return 0; // frontier kinds are priced by frontierProjectSpec (needs world state)
}
function activeProjectCount(w, civId, kind) {
    return w.projects.filter((p) => p.civId === civId && p.kind === kind && !['operational', 'cancelled'].includes(p.status)).length;
}
function projectExists(w, civId, kind) {
    return activeProjectCount(w, civId, kind) > 0;
}
export function proposeProject(w, c, kind, capacity, rationale) {
    ...
    if (capacity <= 0 || activeProjectCount(w, c.id, kind) >= limit)
        return false;
    ...
    if (kind === 'housing') {
        ...
        if (room < capacity) {
            capacity = Math.max(0, room);
            if (capacity < 0.004)
                return false;
            capex = projectCapex(kind, capacity);
        }
    ...
    if (kind === 'solar')
        capex *= clamp(w.techs.solar_pv.cost, 0.35, 1.5);
    if (kind === 'wind')
        capex *= clamp(w.techs.wind_power.cost, 0.4, 1.5);
    if (kind === 'nuclear')
        capex *= clamp(w.techs.nuclear_power.cost, 0.55, 1.8);
    if (kind === 'geothermal')
        capex *= clamp(w.techs.geothermal_power.cost, 0.35, 2.0) / Math.sqrt(w.params.geothermalMult);
    if (kind === 'bioenergy')
        capex *= clamp(w.techs.bioenergy_systems.cost, 0.45, 1.8) / Math.sqrt(w.params.bioenergyMult);
    if (kind === 'ocean_energy')
        capex *= clamp(w.techs.ocean_energy.cost, 0.45, 2.5) / Math.sqrt(w.params.oceanEnergyMult);
    if (kind === 'hydrogen_hub')
        capex *= clamp(w.techs.hydrogen_systems.cost, 0.4, 2.5) / Math.sqrt(w.params.hydrogenMult);
    if (kind === 'storage')
        capex *= clamp(w.techs.storage_batt.cost, 0.3, 1.5);
    if (kind === 'grid')
        capex *= clamp(w.techs.grid_transmission.cost, 0.5, 1.4);
    ...
    if (room < annualNeed) {
        ...
        if (scale < 0.2 || FRONTIER_KINDS.includes(kind) && kind !== 'fusion' && kind !== 'recycling')
            return false;
        capacity *= scale;
        capex *= scale;
    }
...
function completeProject(w, p) {
    ...
    if (applyFrontierCompletion(w, c, p) || applyLandCompletion(w, c, p)) {
        ...
        const source = c.energy.sources[sourceKey];
...
function completionText(p) {
    switch (p.kind) {
        ...
        case 'subsea_habitat':
        case 'aerial_platform': return `${p.capacity.toFixed(2)} capacity units`;
        ...
        case 'mars_colony':
        case 'interstellar_ark': return `Living space for ${(p.capacity * 1000).toFixed(0)} thousand people`;
        case 'space_elevator': return 'A permanent tether to geostationary orbit';
        default: return `${p.capacity.toFixed(2)} GW`;
    }
}
function stepProjects(w) {
    for (const p of w.projects) {
        if (p.status === 'operational' || p.status === 'cancelled')
            continue;
        const c = w.civs.find((x) => x.id === p.civId);
        if (p.status === 'financed') {
            p.status = 'under_construction';
            p.startedAt ??= w.tMonths;
        }
        ...
        if (p.spent >= p.capex * 0.98 && p.monthsRemaining <= 0)
            completeProject(w, p);
    }
    if (w.projects.length > 250)
        w.projects = w.projects.filter((p) => p.status !== 'operational' || (p.completedAt ?? 0) > w.tMonths - 360);
}
function planningScores(w, c) {
...
function rebalanceResearch(w, c) {
    ...
    for (const id of TECH_IDS) {
        ...
        if (['solar_pv', 'wind_power', 'nuclear_power', 'geothermal_power', 'bioenergy_systems', 'ocean_energy', 'hydrogen_systems', 'storage_batt', 'grid_transmission'].includes(id))
            v *= 1 + scores.powerGap * 6 + scores.gridNeed * 1.8;
        if (id === 'geothermal_power')
            v *= w.params.geothermalMult * (1 + scores.powerGap * 2.2);
        if (id === 'bioenergy_systems')
            v *= w.params.bioenergyMult * (1 + scores.powerGap * 1.1 + Math.max(0, w.resources.fossilCostIndex - 1) * 0.8);
        if (id === 'ocean_energy')
            v *= w.params.oceanEnergyMult * w.params.marineMult * (1 + c.land.pressure * 0.8);
        if (id === 'hydrogen_systems')
            v *= w.params.hydrogenMult * (1 + Math.max(0, c.energy.sources.solar.cap + c.energy.sources.wind.cap - c.energy.demandTWh / 8.76 * 0.5) * 0.01);
        if (['ai_models', 'ai_agents', 'compute_accel', 'semiconductor_fab'].includes(id))
            v *= 1 + c.traits.sciencePriority * 0.8 + scores.computeNeed * 1.2;
        if (['robotics_ind', 'robotics_gp'].includes(id))
            v *= 1 + (1 - c.population.workingShare) * 1.4 + Math.max(0, c.economy.unemployment < 0.04 ? 0.4 : 0);
        // The marine-platform pathway lives in industrial robotics: a full island pulls research toward it.
        if (id === 'robotics_ind')
            v *= 1 + clamp((c.land.pressure - 0.8) / 0.2, 0, 1) * 0.5 * w.params.marineMult + (w.params.marineMult - 1) * 0.3;
        if (id === 'biotech_med')
            v *= 1 + Math.max(0, c.population.medianAge - 38) * 0.025;
        ...
        if (id === 'longevity_bio')
            v *= (1 + Math.max(0, c.population.medianAge - 36) * 0.05 + (c.population.total < c.population.peakPhysical * 0.9 ? 0.8 : 0)) * w.params.longevityMult * (0.4 + c.economy.outputPerCapita * 0.4) * earned;
        if (id === 'fusion_power')
            v *= (1 + scores.powerGap * 4 + Math.max(0, w.resources.fossilCostIndex - 1.2) * 1.5 + c.traits.sciencePriority * 0.6) * w.params.fusionMult * earned;
        if (id === 'space_systems')
            v *= (1 + Math.max(0, w.resources.mineralCostIndex - 1.2) * 1.2 + w.resources.landPressure * 0.8 + c.traits.strategicAutonomy * 0.5 + c.traits.riskTolerance * 0.4) * w.params.spaceMult * (0.5 + Math.min(2, c.economy.outputPerCapita) * 0.35) * earned;
        if (id === 'semiconductor_fab')
            v *= 1 + c.traits.strategicAutonomy * 0.8;
        raw[id] = Math.max(0.001, v);
        sum += raw[id];
    }
    for (const id of TECH_IDS) {
    ...
    for (const id of TECH_IDS)
        c.researchAlloc[id] /= norm;
}
function stepCivilizationPlanning(w) {
    if (!(w.tMonths === 1 || w.tMonths % 12 === 0))
        return;
    for (const c of w.civs) {
        const ec = c.economy;
        ...
        const agingRisk = Object.keys(c.energy.sources).reduce((mx, k) => {
            const src = c.energy.sources[k];
            const life = CALIBRATION.generation[k].lifetimeYears;
        ...
        if (!gridBound && (sc.powerGap > 0.02 || c.energy.marginPct < 14 || agingRisk > 0.62)) {
            ...
            for (const k of Object.keys(c.energy.sources)) {
                const src = c.energy.sources[k];
            ...
            const fastPreferred = solarFirmCost <= windFirmCost ? 'solar' : 'wind';
            ...
            if (needTWh > c.energy.demandTWh * 0.01)
                proposeProject(w, c, fastPreferred, Math.min(fastNeedGW, maxFast), c.energy.marginPct < 4 ? 'urgent power shortage and near-term retirement risk' : 'reserve-margin and fleet-replacement requirement');
            ...
            if (c.energy.marginPct < 4 || expectedRetirementTWh > c.energy.demandTWh * 0.08) {
                const second = fastPreferred === 'solar' ? 'wind' : 'solar';
        ...
        if (sc.gridNeed > 0.10 || c.energy.gridCondition < 0.88 || c.energy.marginPct < 8) {
            ...
            const expansion = Math.max(3, Math.min(Math.max(c.energy.gridCapGW * 0.16, gridGapGW * 0.38), Math.max(18, (c.energy.demandTWh / 8.76) * 0.28)));
        ...
        if (c.housing.crowding > 0.08)
            proposeProject(w, c, 'housing', clamp(c.housing.crowding * 0.16, 0.015, 0.12), c.land.pressure > 0.9 ? 'high rents: building higher on the last free land' : 'high rents and insufficient usable housing');
        ...
        if (c.compute.constrained && c.energy.marginPct > 8)
            proposeProject(w, c, 'datacenter', clamp(c.compute.dcCapGW * 0.12, 0.2, 2.5), 'compute demand exceeds available data-center capacity');
        if (c.traits.strategicAutonomy > 0.65 && w.techs.semiconductor_fab.mfg < 2.5 && !projectExists(w, c.id, 'fab'))
            proposeProject(w, c, 'fab', 1, 'strategic dependence on imported leading-edge semiconductors');
    }
}
function fusionBuildable(w) {
 ...
 */
function stepBasicProvisionPolicy(w, c) {
    const ec = c.economy;
    const s = c.society;
...
function createInventor(w, c, program) {
...
function maybeStartResearchPrograms(w, c) {
    ...
    for (const { id } of ranked) {
        if (started >= 2 || active >= CALIBRATION.research.maxActiveProgramsPerCiv)
            break;
        const t = w.techs[id];
        ...
        if (!candidate || t.cap < t.paradigmCap * candidate.minCapFraction)
            continue;
        if (w.researchPrograms.some((p) => p.techId === id && p.civId === c.id && p.status !== 'failed' && p.status !== 'cancelled' && p.status !== 'completed'))
            continue;
...
function stepResearchPrograms(w) {
    if (w.tMonths === 1 || w.tMonths % 12 === 0)
        for (const c of w.civs)
            maybeStartResearchPrograms(w, c);
    for (const p of w.researchPrograms) {
        ...
        if (p.status === 'active') {
            ...
            if (p.progress >= 1) {
                ...
                p.breakthroughAt = w.tMonths;
                const lead = createInventor(w, c, p);
                p.leadCharacterId = lead;
                const inv = {
                    ...
                    leadCharacterId: lead, sourceProgramId: p.id, status: 'discovered',
                };
                ...
                if (duplicate) {
                    ...
                    t.paradigm += 1;
                    t.maturity *= 0.82;
                    t.reliability *= 1 - candidate.reliabilityPenalty;
                    t.cost *= candidate.costPenalty;
                }
                addEvent(w, {
            ...
            if (p.maturity >= 1) {
                ...
                if (inv) {
                    inv.status = 'deployed';
                    inv.validatedAt = w.tMonths;
                    inv.deployedAt = w.tMonths;
                }
                t.maturity = Math.max(t.maturity, 0.58);
                t.reliability = Math.max(t.reliability, 0.55);
                t.mfg *= 1.05;
                addEvent(w, {
...
function generationTWh(w, c) {
    ...
    for (const k of Object.keys(c.energy.sources)) {
        ...
        let techGain = 1;
        if (k === 'fossil')
            techGain *= fossilDispatch;
        if (k === 'fusion')
            techGain *= 0.85 + Math.min(0.2, Math.log1p(Math.max(0, w.techs.fusion_power.cap - 1)) * 0.1);
        if (k === 'solar')
            techGain += Math.min(0.35, Math.log1p(Math.max(0, w.techs.solar_pv.cap - 1)) * 0.10);
        if (k === 'wind')
            techGain += Math.min(0.28, Math.log1p(Math.max(0, w.techs.wind_power.cap - 1)) * 0.08);
        if (k === 'nuclear')
            techGain += Math.min(0.12, Math.log1p(Math.max(0, w.techs.nuclear_power.cap - 1)) * 0.04);
        if (k === 'geothermal')
            techGain += Math.min(0.18, Math.log1p(Math.max(0, w.techs.geothermal_power.cap - 1)) * 0.06);
        if (k === 'bioenergy')
            techGain += Math.min(0.12, Math.log1p(Math.max(0, w.techs.bioenergy_systems.cap - 1)) * 0.04);
        if (k === 'ocean')
            techGain += Math.min(0.20, Math.log1p(Math.max(0, w.techs.ocean_energy.cap - 1)) * 0.07);
        g += src.cap * src.condition * CAPACITY_FACTOR[k] * techGain * 8.76;
    }
    return g;
}
function deliverableTWh(w, c, genTWh) {
...
function allocateElectricity(c, deliverable, totalDemand) {
    const e = c.energy;
    ...
    const critical = Math.min(criticalReq, remaining);
    remaining -= critical;
    // Keep a household floor before commercial rationing.
    const householdFloorReq = householdReq * 0.65;
    const householdFloor = Math.min(householdFloorReq, remaining);
    remaining -= householdFloor;
...
function stepEnergy(w) {
    for (const c of w.civs) {
        ...
        const h2 = e.hydrogen;
        const cleanGen = Object.entries(e.sources).reduce((sum, [k, src]) => k === 'fossil' ? sum : sum + src.cap * src.condition * CAPACITY_FACTOR[k] * 8.76, 0) + e.spaceSolarTWh;
...
function stepFood(w) {
    ...
    for (const c of w.civs) {
        ...
        const yieldTarget = 1 + Math.max(0, bioCap - 1) * 0.28 +
        ...
        if ((bioCap > 1.3 || f.artificialShare > 0.05) && !f.artificialUnlocked)
            f.artificialUnlocked = true;
...
function stepUrban(w) {
    ...
    for (const c of w.civs) {
        ...
        const habitatOutage = c.land.floating * (1 - c.land.floatingCondition) +
...
function stepEconomy(w) {
    for (const c of w.civs) {
        ...
        if (ec.publicDebt > 3 && c.society.stability < 0.45)
            ec.publicDebt *= 1 - 0.08 * DT;
...
function stepPopulation(w) {
    for (const c of w.civs) {
        ...
        p.birthRate = rates.birthRate;
        p.deathRate = rates.deathRate;
    ...
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            ...
            const pull = clamp((b.economy.wageIndex / Math.max(0.2, a.economy.wageIndex) - 1) * 0.5 +
                ...
                (b.society.basicProvision - a.society.basicProvision) * 0.25, -0.5, 0.5);
            if (pull > 0.05) {
...
function stepSociety(w) {
    for (const c of w.civs) {
        ...
        const trustTarget = clamp(-Math.max(0, c.economy.gini - 0.42) * 0.35 +
            ...
            - severeEnergyStress * 0.28 - c.food.shortage * 0.48 - c.waste.accumulation * 0.12 - c.housing.abandoned * 0.15, 0.08, 0.90);
...
function stepDiplomacy(w) {
    const aiGap = (a, b) => w.techs.ai_models.adoption[a].intensity * w.techs.ai_models.cap - w.techs.ai_models.adoption[b].intensity * w.techs.ai_models.cap;
    for (const r of w.relations) {
        ...
        const foodBind = Math.min(ca.food.importShare, Math.max(0, cb.food.selfSufficiency - 1)) +
        ...
        if (r.tension > 0.75 && r.relation < -0.35 && !w.flags[`war-${r.a}-${r.b}`] && chance(w.rng, 0.008)) {
            ...
            let largest = 'fossil';
            for (const k of Object.keys(weaker.energy.sources)) {
                if (weaker.energy.sources[k].cap > weaker.energy.sources[largest].cap)
                    largest = k;
            }
...
function stepEnvironment(w) {
    ...
    for (const c of w.civs) {
        ...
        let g = f;
        for (const k of ['nuclear', 'solar', 'wind', 'hydro'])
            g += c.energy.sources[k].cap * CAPACITY_FACTOR[k];
        fossilShare += f;
        totalGen += g;
    }
    ...
    for (const [thr, key, title] of [[2.0, 'warming-2.0', 'Warming passes 2°C'], [2.5, 'warming-2.5', 'Warming passes 2.5°C'], [3.0, 'warming-3.0', 'Warming passes 3°C']]) {
...
function stepEvents(w) {
    ...
    for (const c of w.civs) {
        ...
        if (mPrevYear && c.economy.output > mPrevYear.output[c.id] * 1.005)
            w.flags[key('recession')] = false;
        ...
        if (energyHardship(c) < 0.1)
            w.flags[key('exodus')] = false;
        // ── food events ──
        const f = c.food;
        ...
        if (hs.crowding < 0.08)
            w.flags[key('housing-crisis')] = false;
        const ws = c.waste;
...
function stepResourceEvents(w) {
    const r = w.resources;
    const once = (key, cond, make) => {
        if (cond && !w.flags[key]) {
            w.flags[key] = true;
            addEvent(w, make());
        }
    ...
    for (const ch of w.characters) {
        ...
        if (yearly && ch.age >= retireAge && ch.retiredAt == null) {
            ...
            if (chance(w.rng, retireChance)) {
                ...
                ch.prominence *= 0.62;
                if (!ch.role.startsWith('Retired '))
                    ch.role = `Retired ${ch.role}`;
        ...
        ch.mood = clamp(0.3 * (1 - c.economy.unemployment * 4) + 0.3 * (1.4 - c.energy.priceIndex) + 0.4 * c.society.trust - 0.5 - c.food.shortage * 0.6 - c.housing.crowding * 0.2 - Math.min(0.3, c.waste.accumulation * 0.2), -1, 1);
        // prominence rises when their domain is in the news
        const domainHot = (ch.id === 'elena_voss' && c.energy.constrained) ||
...
function recordMetrics(w) {
    if (w.tMonths % 6 !== 0)
        return; // every 6 months
    const t = w.techs;
    const rec = { veloria: 0, ardan: 0, nemea: 0 };
    const grab = (fn) => {
        for (const c of w.civs)
            rec[c.id] = fn(c);
        return { ...rec };
    ...
    w.metrics.push({
        ...
        researchVel: grab((c) => TECH_IDS.reduce((a, id) => a + c.researchAlloc[id] * w.techs[id].lastGrowthRate, 0) * 100),
    ...
    if (w.metrics.length > 800)
        w.metrics.splice(0, w.metrics.length - 800);
...
export function fmtHorizon(hrs) {
    if (hrs >= 15.99)
        return '≥16 hours (measurement ceiling)';
    if (hrs < 48)
        return `${hrs.toFixed(0)} hours`;
    if (hrs < 720)
        return `${(hrs / 24).toFixed(0)} days`;
    if (hrs < 8760)
        return `${(hrs / 720).toFixed(1)} months`;
    return `${(hrs / 8760).toFixed(1)} years`;
}
// ── ORCHESTRATOR ────────────────────────────────────────────────────────────
function decayInterventions(w) {
    for (const iv of w.activeInterventions)
        iv.monthsLeft -= 1;
    const expired = w.activeInterventions.filter((iv) => iv.monthsLeft <= 0);
    w.activeInterventions = w.activeInterventions.filter((iv) => iv.monthsLeft > 0 || iv.effects.some((e) => e.kind === 'build' || e.kind === 'relation' || e.kind === 'society'));
...
function literalWorldPopulation(w) {
 ...
 */
function stepPostCivilization(w) {
    w.pendingDecisions = [];
    w.pendingConflict = [];
    w.conflicts = [];
    ...
    for (const r of w.researchPrograms)
        if (r.status === 'active' || r.status === 'validation' || r.status === 'breakthrough')
            r.status = 'cancelled';
    for (const ch of w.characters)
        ch.active = false;
    ...
    for (const c of w.civs) {
        ...
        c.economy.output = 0;
        c.economy.outputPerCapita = 0;
        c.economy.laborForce = 0;
        c.economy.unemployment = 0;
        c.economy.capexAvailability = 0;
        c.researchCapacity = 0;
        c.frontierReadiness = 0;
        c.compute.demandFlops = 0;
        c.energy.demandTWh = 0;
        c.energy.servedTWh = 0;
        c.energy.unservedTWh = 0;
        c.energy.marginPct = 0;
        c.energy.criticalServedRatio = 0;
        c.energy.industryServedRatio = 0;
        c.energy.computeServedRatio = 0;
...
export function stepWorld(w, months = 1) {
