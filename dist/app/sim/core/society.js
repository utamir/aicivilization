...
const DT = 1 / 12;
const deployed = (w, prefix) => w.inventions.some((i) => i.id.startsWith(`inv-${prefix}-`) && i.status === 'deployed');
const rnd = (w) => nextFloat(w.rng);
const rel = (w, a, b) => w.relations.find((r) => (r.a === a && r.b === b) || (r.a === b && r.b === a));
export function stepSocietyDynamics(w) {
...
function transferEmbodiedPeople(donor, recipient, requestedM) {
...
function continuityAndRecovery(w) {
    ...
    for (const c of w.civs) {
        ...
        if (earth < ABANDONMENT_M) {
            ...
            if (allLiving > 1e-6 && w.params.recoveryMult > 0.25) {
                ...
                if (earth < RESETTLEMENT_M && c.population.digitalShare > 0.25 && w.techs.robotics_gp.cap > 2.4 && w.techs.biotech_med.cap > 1.8 && s.autonomousInfrastructure > 0.35) {
                    ...
                    if (embodied > 0)
                        c.population.digitalShare = clamp(c.population.digitalShare - embodied / Math.max(1e-9, c.population.total), 0, 1);
                }
        ...
        if (s.stability < 0.16 && c.economy.institutionalCapacity < 0.18)
            s.polityStatus = 'failed';
        else if (s.polityStatus === 'failed' && s.stability > 0.34 && c.economy.institutionalCapacity > 0.30)
            s.polityStatus = 'active';
    }
}
// ── Synthetic births ────────────────────────────────────────────────────────
export function syntheticBirthsAvailable(w) {
    return deployed(w, 'in-vitro-gametogenesis') && deployed(w, 'full-ectogenesis');
}
function syntheticBirths(_w, c) {
    const s = c.society;
    if (s.syntheticProgram === 'none' || s.syntheticProgram === 'ban')
        return;
    ...
    let programme = 0;
    if (s.syntheticProgram === 'state' || s.syntheticProgram === 'both')
        programme += Math.max(0, replacementBirths - natural) * 0.7;
    if (s.syntheticProgram === 'family' || s.syntheticProgram === 'both')
        programme += p.total * 0.0025 * clamp(c.economy.outputPerCapita / 2, 0.2, 1.5);
...
function contagion(w, c) {
    ...
    for (const o of others) {
        ...
        pull += (o.population.fertility - c.population.fertility) * link;
        weight += link;
    }
    if (weight <= 0)
        return;
        ...
        if (war) {
            ...
            for (const c of [a, b]) {
                ...
                for (const k of Object.keys(c.energy.sources))
                    c.energy.sources[k].cap *= 1 - 0.004 * share * DT * 12;
                c.housing.stockIndex *= 1 - 0.006 * share * DT * 12;
                c.housing.condition = clamp(c.housing.condition - 0.01 * DT * 12, 0.2, 1);
                c.economy.publicDebt += 0.06 * DT * 12;
                c.economy.capexAvailability *= 1 - 0.01 * DT * 12;
            ...
            r.tradeOpenness = Math.min(r.tradeOpenness, 0.1);
            r.sciCollaboration = Math.min(r.sciCollaboration, 0.05);
            r.relation = Math.min(r.relation, -0.6);
            ...
            if (war.months > 24 && (a.society.stability < 0.2 || b.society.stability < 0.2 || rnd(w) < 0.02)) {
                ...
                r.tension = 0.35;
                r.grievances.push(`war of ${Math.floor(yearOf(w))}`);
...
export function startWar(w, a, b) {
    if (w.conflicts.some((k) => (k.a === a && k.b === b) || (k.a === b && k.b === a)))
        return;
...
function pandemic(w) {
    ...
    if (P) {
        ...
        for (const c of w.civs) {
            ...
            const co = c.population.cohorts;
            let dead = 0;
            for (let i = 0; i < co.length; i++) {
                const wgt = i >= 13 ? 3 : i >= 10 ? 1.2 : 0.5;
                const d = co[i] * kill * wgt;
                co[i] -= d;
                dead += d;
            }
            c.population.total -= dead;
            c.economy.capexAvailability *= 1 - c.society.lockdown * 0.02 * DT * 12;
            c.economy.output *= 1 - c.society.lockdown * 0.015 * DT * 12;
...
function gerontocracy(w, c) {
    ...
    if (youngest !== c) {
        ...
        let moved = 0;
        for (let i = 3; i < 8; i++) {
            const m = co[i] * old * 0.004 * DT;
            co[i] -= m;
            moved += m;
        } // 15–39 leave
        c.population.total -= moved;
        youngest.population.total += moved;
    }
...
function brainDrain(w) {
    ...
    for (const c of w.civs) {
        ...
        if (loss > 0.01 && c.society.stability < 0.45) {
            ...
            if (best !== c)
                best.researchCapacity *= 1 + drain * 0.6;
        }
    }
}
// ── Secession ───────────────────────────────────────────────────────────────
function secession(w, c) {
    ...
    if (c.society.stability < 0.15 && !c.society.fragmented) {
        ...
        if (c.society.lowStabilityMonths > 36) {
            c.society.fragmented = true;
            w.flags[key] = true;
            c.economy.institutionalCapacity *= 0.6;
            c.economy.output *= 0.85;
            c.researchCapacity *= 0.7;
...
function revival(w, c) {
    ...
    if (c.society.revivalMonths > 0) {
        c.society.revivalMonths -= 1;
        c.population.fertility = lerp(c.population.fertility, c.population.fertility + 0.45, 0.004);
    }
}
// ── Drought ─────────────────────────────────────────────────────────────────
function drought(w) {
    if (w.drought) {
        w.drought.months -= 1;
        for (const c of w.civs) {
            c.food.landYield *= 1 - 0.02 * DT * 12 * civDef(c.id).food.climateSensitivity;
        }
        if (w.drought.months <= 0) {
            w.drought = null;
            for (const c of w.civs)
                c.food.landYield *= 1.12;
        }
        return;
    }
...
// ── AI incident ─────────────────────────────────────────────────────────────
function aiIncident(w) {
    if (!w.frontier.agi || w.flags['ai-incident'])
        return;
    if (w.tMonths % 12 === 0 && rnd(w) < 0.03 / Math.max(0.5, w.params.aiSafety)) {
        w.flags['ai-incident'] = true;
        for (const c of w.civs) {
            c.society.backlash = clamp(c.society.backlash + 0.15 * (1 - c.traits.riskTolerance), 0, 1);
            w.techs.ai_agents.adoption[c.id].penetration *= 0.85;
        }
...
// ── Solar storm ─────────────────────────────────────────────────────────────
function solarStorm(w) {
    if (w.tMonths % 12 !== 0 || rnd(w) > 0.008)
        return; // Carrington-class every ~125 years
    for (const c of w.civs) {
