...
export function mergeDeltas(...ds) {
    const out = {};
    for (const d of ds)
        Object.assign(out, d);
    return out;
...
export function parseWhatIf(text) {
    const t = text.trim();
    if (t.length < 4)
        return { ok: false, label: '', delta: {}, explanation: 'Describe an assumption to change — e.g. “AI improves rapidly but electricity grows slowly”.' };
