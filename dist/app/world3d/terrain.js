...
import { WORLD_SIZE, CITIES } from './layout.js';
...
export function buildTerrain() {
    ...
    for (let i = 0; i < pos.count; i++) {
        ...
        if (h < SEA_LEVEL + 0.5)
            ...
            c = cSnow.clone().lerp(cRock, 0.4);
        else if (h > 8.5)
            c = cRock.clone().lerp(cForest, THREE.MathUtils.clamp((12 - h) / 4, 0, 0.6));
        else {
            c = cGrass.clone().lerp(cMeadow, fbm(x / 15, z / 15, 2));
            if (moist > 0.58)
                c.lerp(cForest, (moist - 0.58) * 2.4);
            // Nemea (south) drier
