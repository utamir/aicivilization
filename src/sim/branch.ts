// ─────────────────────────────────────────────────────────────────────────────
// INTERVENTIONS APPLICATION + SNAPSHOT / BRANCH / COMPARE
// ─────────────────────────────────────────────────────────────────────────────
import type { WorldState, CivId, ProjectKind } from './types';
import { INTERVENTIONS } from './data/interventions';
import { addEvent, taskHorizonHrs } from './core/step';
import { cloneWorld } from './core/world';
import { CALIBRATION } from './data/calibration';

function interventionProjectSpec(kind: ProjectKind, capacity: number) {
  switch (kind) {
...
function queueInterventionProject(w: WorldState, civId: CivId, kind: ProjectKind, capacity: number, rationale: string) {
...
export function applyIntervention(
  ...
  if (w.observerBudget < def.cost) return { ok: false, reason: 'Not enough intervention budget' };
  ...
  for (const ef of effects) {
    if (ef.kind === 'build') {
      const kind: ProjectKind = ef.buildType;
      ...
      if (r) {
        if (ef.field === 'chipExportAllowed') {
          ...
          if (!ef.value) r.grievances.push('export controls');
        } else {
          (r as any)[ef.field] = Number(ef.value);
        }
      }
    }
  }
...
export function createBranch(w: WorldState, note: string, change: (b: WorldState) => void): BranchResult {
...
export function compareWorlds(a: WorldState, b: WorldState): CompareRow[] {
  const sum = (w: WorldState, fn: (c: WorldState['civs'][0]) => number) => w.civs.reduce((x, c) => x + fn(c), 0);
  const mean = (w: WorldState, fn: (c: WorldState['civs'][0]) => number) => sum(w, fn) / w.civs.length;
