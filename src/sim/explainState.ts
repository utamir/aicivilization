// ─────────────────────────────────────────────────────────────────────────────
// EXPLAIN STATE — the same arithmetic the simulation uses, laid out for a
// player who asks "why is stability 61%?" Nothing here changes state.
// ─────────────────────────────────────────────────────────────────────────────
import type { CivState } from './types';
import { civDef } from './data/civDefs';

export interface Factor { label: string; value: number; note?: string }

/** Contributions to the stability target (sum ≈ target before clamping). */
export function explainStability(c: CivState): { target: number; factors: Factor[] } {
...
export function explainReadiness(c: CivState): Factor[] {
  const s = c.society, e = c.economy;
...
export function demographicAnalogue(c: CivState): string {
  const d = civDef(c.id);
...
export function civHeadline(c: CivState, atWar = false): string {
  if (atWar) return 'At war';
  if (c.population.total < civDef(c.id).population.total * 0.01) return 'No longer exists as a civilization';
  if (c.food.shortage > 0.15) return 'Famine';
  if (c.society.fragmented) return 'Fragmented';
  if (c.society.lockdown > 0) return 'Pandemic';
  if (c.society.stability < 0.2) return 'State failure';
  if (c.energy.marginPct < -15) return 'Rolling blackouts';
  if (c.society.stability < 0.4) return 'Unrest';
  if (c.land.pressure > 0.95 && c.housing.crowding > 0.1) return 'Out of land';
  if (c.frontierReadiness >= 0.7) return 'Stable and expanding';
  if (c.frontierReadiness >= 0.5) return 'Stable';
  return 'Under strain';
}
