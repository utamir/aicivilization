// ─────────────────────────────────────────────────────────────────────────────
// DEMOGRAPHY — a real population is a set of cohorts, not one number. Twenty-one
// five-year bands (0–4 … 100+). Births come from the women in the fertile
// bands by an age schedule; deaths follow a Gompertz curve scaled to the
// civilization's life expectancy (longevity therapies flatten the old-age end);
// everyone ages one band every five years. This is what gives momentum: a
// young country keeps growing for decades after fertility falls, an old one
// keeps shrinking after it recovers, and the median age and working share are
// read off the pyramid instead of being assumed.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export function mortalityScale(lifeExpectancy: number): number {
...
export function initialCohorts(totalM: number, medianAge: number): number[] {
  const survival: number[] = [1];
  for (let i = 1; i < BANDS; i++) survival[i] = survival[i - 1] * Math.pow(1 - baseMortality(MID[i - 1]), BAND_YEARS);
  const shape = (k: number) => survival.map((s, i) => s * Math.exp(-k * MID[i]));
  const medianOf = (w: number[]) => {
    const tot = w.reduce((a, b) => a + b, 0); let acc = 0;
    for (let i = 0; i < BANDS; i++) { if (acc + w[i] >= tot / 2) return i * BAND_YEARS + BAND_YEARS * (tot / 2 - acc) / w[i]; acc += w[i]; }
    return 100;
  };
  let lo = -0.05, hi = 0.08;
  for (let it = 0; it < 40; it++) { const k = (lo + hi) / 2; if (medianOf(shape(k)) > medianAge) lo = k; else hi = k; }
  const w = shape((lo + hi) / 2); const tot = w.reduce((a, b) => a + b, 0);
  return w.map((x) => x / tot * totalM);
}

export function medianAgeOf(cohorts: number[]): number {
  const tot = cohorts.reduce((a, b) => a + b, 0); if (tot <= 0) return 30;
  let acc = 0;
  for (let i = 0; i < BANDS; i++) { if (acc + cohorts[i] >= tot / 2) return i * BAND_YEARS + BAND_YEARS * (tot / 2 - acc) / Math.max(1e-9, cohorts[i]); acc += cohorts[i]; }
  return 100;
}

/** Working-age share: 15 up to a retirement age that moves with healthy life (65 at LE 80, ~96 at LE 130). */
export function workingShareOf(cohorts: number[], lifeExpectancy: number): number {
  const tot = cohorts.reduce((a, b) => a + b, 0); if (tot <= 0) return 0;
  const retire = lifeExpectancy <= 80 ? 65 : 40 + 25 * (lifeExpectancy - 40) / 40;
  let w = 0;
  for (let i = 3; i < BANDS; i++) {
    const lo = i * BAND_YEARS, hi = lo + BAND_YEARS;
    if (hi <= retire) w += cohorts[i]; else if (lo < retire) w += cohorts[i] * (retire - lo) / BAND_YEARS;
  }
  return w / tot;
}

/** Scale the pyramid so it sums to `total` (after migration, off-world moves, births added elsewhere). */
export function reconcile(p: PopulationState) {
  const sum = p.cohorts.reduce((a, b) => a + b, 0);
  if (sum <= 0) {
    if (p.total > 1e-6) p.cohorts = initialCohorts(p.total, p.medianAge);
    else { p.total = 0; p.cohorts = Array(BANDS).fill(0); p.workingShare = 0; }
    return;
  }
  const f = p.total / sum;
  if (Math.abs(f - 1) > 1e-9) for (let i = 0; i < BANDS; i++) p.cohorts[i] *= f;
}
 ...
 */
export function stepCohorts(p: PopulationState, extraDeath: number, birthSuppression: number, dt: number): { birthRate: number; deathRate: number } {
  const c = p.cohorts;
  const total = c.reduce((a, b) => a + b, 0);
  if (total <= 0) return { birthRate: 0, deathRate: 0 };
  // births
  let births = 0;
  for (let k = 0; k < ASFR_SHAPE.length; k++) births += c[3 + k] * FEMALE * p.fertility * ASFR_SHAPE[k] / BAND_YEARS;
  // longevity therapies extend the fertile window a little (45–54)
  if (p.lifeExpectancy > 95) births += (c[9] + c[10]) * FEMALE * p.fertility * 0.03 / BAND_YEARS;
  ...
  for (let i = BANDS - 1; i >= 1; i--) { const x = c[i - 1] * move; c[i - 1] -= x; c[i] += x; }
  c[0] += births;
  p.total = Math.max(0, c.reduce((a, b) => a + b, 0));
  if (p.total < 1e-6) { p.total = 0; for (let i = 0; i < BANDS; i++) c[i] = 0; }
...
export function ageGroups(p: PopulationState): number[] {
  const c = p.cohorts; const tot = Math.max(1e-9, c.reduce((a, b) => a + b, 0));
  const sum = (a: number, b: number) => c.slice(a, b).reduce((x, y) => x + y, 0) / tot;
