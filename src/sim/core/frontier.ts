// ─────────────────────────────────────────────────────────────────────────────
// FRONTIER — everything beyond the 2026 envelope: fusion as a buildable
// option, launch economics, off-Earth settlement, digital minds, the
// Kardashev index and the milestone ladder. All of it is driven by the same
// research → invention → project pipeline as the rest of the world. There is
// no "tech tree unlock" that fires on a date.
// ─────────────────────────────────────────────────────────────────────────────
import type { WorldState, CivState, CivId, ProjectKind, ProjectState } from '../types';
import { CALIBRATION } from '../data/calibration';
import { civDef } from '../data/civDefs';
import { MILESTONES, ERA_LABELS, milestoneDef } from '../data/frontier';
import { addEvent, yearOf } from './step';
import { clamp, clamp01, lerp } from './formulas';
import { chance } from '../rng';

const DT = 1 / 12;

/** People who physically live on Earth in bodies: they need houses, food and land. */
export function physicalPopulation(c: CivState): number {
 ...
 */
export function computeFrontierReadiness(c: CivState): number {
  ...
  if (physicalPopulation(c) < 0.001 || c.society.polityStatus === 'abandoned') return 0;
...
export function agiToolingBoost(w: WorldState): number {
  return 1 + (w.frontier.agi ? 0.55 : 0) + (w.frontier.asi ? 1.1 : 0);
}

export type ProposeFn = (w: WorldState, c: CivState, kind: ProjectKind, capacity: number, rationale: string) => boolean;

// ── planning: which frontier projects a civilization proposes this year ────
export function planFrontierProjects(w: WorldState, c: CivState, propose: ProposeFn) {
  ...
  if (physicalPopulation(c) < 0.001 || c.society.polityStatus === 'abandoned') return;
  const t = w.techs;
  const sp = c.space;
  ...
  if (spaceReady > 0.35 && t.space_systems.reliability > 0.5 && wellEarned && (wealth > 1.15 || c.traits.strategicAutonomy > 0.7 || programPush > 1)) {
    ...
    if (want > 0) propose(w, c, 'spaceport', want * programPush, sp.spaceportCapacity < 0.5 ? 'establish reusable heavy-launch capacity' : 'expand launch cadence for orbital industry');
  }
  ...
  if (closedLoop && sp.launchCostIndex <= 0.08 && sp.orbitalIndustry >= 0.5 && wellEarned && wealth > 1.4 && (landPush > 0.5 || programPush > 1 || sp.habitatCapacityM > 0 || wealth > 3)) {
    ...
    if (occupancy > 0.7) propose(w, c, 'orbital_habitat', next, sp.habitatCapacityM === 0 ? 'first permanent rotating habitat' : 'orbital habitat demand exceeds capacity');
  }
  ...
  if (closedLoop && sp.launchCostIndex <= 0.1 && sp.spaceportCapacity >= 1.5 && t.space_systems.reliability >= 0.6 && wellEarned && (c.traits.riskTolerance > 0.55 || programPush > 1)) {
    ...
    if (sp.marsCapacityM > 0 && landPush < 0.5 && programPush <= 1) return;
    const occupancy = sp.marsCapacityM > 0 ? Math.min(1, marsPopulation(c) / Math.max(0.01, sp.marsCapacityM)) : 1;
    if (occupancy > 0.6 && (sp.marsCapacityM === 0 || w.frontier.milestones.find((m) => m.id === 'terraforming')?.status !== 'locked')) propose(w, c, 'mars_colony', next, sp.marsCapacityM === 0 ? 'first permanent settlement on Mars' : 'expand Mars settlement');
  }
  ...
  if (planetary && closedLoop && sp.orbitalIndustry > 8 && sp.habitatCapacityM + sp.marsCapacityM > 2 && c.population.offworld > 0.5 && t.space_systems.cap > 5 && t.space_systems.reliability > 0.8 && wellEarned && c.economy.outputPerCapita > 4) {
    ...
    if (occupied > 0.7) propose(w, c, 'interstellar_ark', next, sp.deepSpaceCapacityM <= 0.001 ? 'first self-sufficient interstellar seed mission' : 'expand interstellar settlement capacity');
  }
}

function civPolicyStrength(w: WorldState, civId: CivId, policy: string): number {
  let s = 0;
  for (const iv of w.activeInterventions) for (const ef of iv.effects) if (ef.kind === 'policy' && ef.civId === civId && ef.policy === policy) s += ef.strength;
  return s;
}

// ── project completion effects for frontier kinds ───────────────────────────
export function applyFrontierCompletion(w: WorldState, c: CivState, p: ProjectState): boolean {
  const sp = c.space;
  switch (p.kind) {
    case 'fusion': {
    ...
    case 'spaceport': sp.spaceportCapacity += p.capacity; sp.orbitalIndustry += p.capacity * 0.15; return true;
    case 'asteroid_mining': sp.mineralInflow += p.capacity; sp.orbitalIndustry += p.capacity * 4; return true;
    case 'orbital_habitat': sp.habitatCapacityM += p.capacity; sp.orbitalIndustry += p.capacity * 0.6; return true;
    case 'mars_colony': sp.marsCapacityM += p.capacity; sp.orbitalIndustry += p.capacity * 0.4; return true;
    case 'interstellar_ark': sp.deepSpaceCapacityM += p.capacity; sp.orbitalIndustry += p.capacity * 0.2; return true;
    case 'space_elevator': sp.elevator = true; sp.orbitalIndustry += 2; return true;
    case 'power_satellite': sp.powerSatelliteGW += p.capacity; sp.orbitalIndustry += p.capacity * 0.05; return true;
    case 'recycling': return true; // counted directly from operational projects in resources.ts
    default: return false;
  }
}

export function frontierProjectSpec(w: WorldState, c: CivState, kind: ProjectKind, capacity: number): { capex: number; leadMonths: number } | null {
  const sp = c.space;
...
export function reconcileFrontierPopulation(w: WorldState) {
...
export function stepFrontier(w: WorldState) {
  const fr = w.frontier;
  ...
  for (const c of w.civs) {
    ...
    if (sp.deepSpaceCapacityM > sp.interstellarM + 0.0001 && c.population.offworld - sp.interstellarM > 0.05) {
      ...
      if (flow > orbitBefore) sp.marsPopulationM = Math.max(0, sp.marsPopulationM - (flow - orbitBefore));
    }
  ...
  for (const ms of fr.milestones) {
    ...
    if (def.kind === 'threshold') {
      if (blockers.length === 0) achieve(w, ms.id);
      else ms.status = 'locked';
      continue;
    }
    // programs: prerequisites hold → available → in progress while funded
    if (blockers.length > 0) { if (ms.status !== 'in_progress') ms.status = 'locked'; continue; }
    if (ms.status === 'locked') {
      ms.status = 'available';
      addEvent(w, {
    ...
    if (ms.status === 'available' || ms.status === 'in_progress') {
      ...
      for (const c of w.civs) {
        ...
        if (c.frontierReadiness < 0.55) continue;
      ...
      if (funded > 0.2) {
        ...
        if (ms.progress >= 1) achieve(w, ms.id);
      }
    }
  }
  fr.agi = fr.milestones.find((m) => m.id === 'agi')?.status === 'achieved';
  fr.asi = fr.milestones.find((m) => m.id === 'asi')?.status === 'achieved';
  if (!wasAgi && fr.agi) for (const c of w.civs) c.economy.automationExposure = clamp(c.economy.automationExposure + 0.15, 0, 0.9);
  if (!wasAsi && fr.asi) for (const c of w.civs) c.researchCapacity *= 1.5;
  ...
  if (terra && terra.status === 'in_progress') {
    for (const c of w.civs) if (c.space.marsCapacityM > 0) c.space.marsCapacityM += (0.15 + terra.progress * 0.8) * DT;
  }
  ...
  fr.era = currentEra(w);
  if (yearly) classifyTrajectory(w);
}

function achieve(w: WorldState, id: string) {
...
function currentEra(w: WorldState): string {
  let era: 1 | 2 | 3 = 1;
  for (const ms of w.frontier.milestones) {
    if (ms.status === 'achieved') { const d = milestoneDef(ms.id); if (d.era > era) era = d.era; }
  }
...
function updateOutcomeProfile(w: WorldState) {
  const mean = (fn: (c: WorldState['civs'][number]) => number) => w.civs.reduce((a, c) => a + fn(c), 0) / Math.max(1, w.civs.length);
  const geo = (xs: number[]) => Math.pow(xs.reduce((a, x) => a * clamp(x, 0.02, 1), 1), 1 / xs.length);
...
function classifyTrajectory(w: WorldState) {
  const fr = w.frontier;
  updateOutcomeProfile(w);
  const m = w.metrics;
  const n = m.length;
  const pick = (k: number) => m[Math.max(0, n - 1 - k)];
  const now = pick(0), ago = pick(40); // 20 years (metrics every 6 months)
  const pop = (x: typeof now) => w.civs.reduce((a, c) => a + x.population[c.id], 0);
  const out = (x: typeof now) => w.civs.reduce((a, c) => a + x.output[c.id], 0);
  ...
  if (!now || !ago || n < 12) { fr.trajectory = 'growth'; fr.trajectoryNote = 'Too early to classify: the first decade sets the baseline.'; return; }
