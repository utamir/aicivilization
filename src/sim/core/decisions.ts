// ─────────────────────────────────────────────────────────────────────────────
// DECISIONS — at turning points the world stops and asks. Each decision is a
// real fork: the options apply policy, money, research or scenario parameters
// through the same channels an intervention uses. Ignore a decision for two
// years and the civilization decides for itself, in character.
// ─────────────────────────────────────────────────────────────────────────────
import type { WorldState, CivState, CivId, InterventionEffect, PendingDecision, SimParams } from '../types';
type ParamDelta = Partial<SimParams>;
import { addEvent, yearOf, proposeProject } from './step';
import { clamp } from './formulas';
import { startWar, syntheticBirthsAvailable } from './society';

export interface DecisionOption {
  id: string;
  label: string;
  consequence: string;           // what the player is told will happen
  effects?: InterventionEffect[]; // routed through activeInterventions
  params?: ParamDelta;            // scenario-level change (multiplied into world params)
  apply?: (w: WorldState, c: CivState) => void; // direct state change
  /** how much a civilization with these traits would like this option (0..1) when the player stays silent */
  affinity: (c: CivState) => number;
}
export interface DecisionDef {
  id: string;
  title: string;
  question: (w: WorldState, c: CivState) => string;
  trigger: (w: WorldState, c: CivState) => boolean;
  options: DecisionOption[];
  months?: number; // deadline before the civ decides itself (default 24)
}
export type { PendingDecision };

type PolicyId = Extract<InterventionEffect, { kind: 'policy' }>['policy'];
const policy = (civId: CivId, p: PolicyId, strength: number, months: number): InterventionEffect => ({ kind: 'policy', civId, policy: p, strength, months });
...
export const DECISIONS: DecisionDef[] = [
  ...
  },
  {
    ...
    months: 12,
    options: [
      { id: 'negotiate', label: 'Negotiate: share the scarce thing', consequence: 'A treaty on minerals, food or water. Tension falls; you give up some of what you have; collaboration improves.', apply: (w, c) => { const k = w.pendingConflict.shift(); if (!k) return; const r = w.relations.find((x) => (x.a === k.a && x.b === k.b) || (x.a === k.b && x.b === k.a))!; r.tension = 0.3; r.relation = Math.min(1, r.relation + 0.3); r.tradeOpenness = Math.min(1, r.tradeOpenness + 0.15); c.economy.output *= 0.985; }, affinity: (c) => c.traits.openness * 0.8 },
      { id: 'sanction', label: 'Sanctions and blockade', consequence: 'Trade and science with them stop. Their economy suffers, yours less; tension stays high.', apply: (w) => { const k = w.pendingConflict.shift(); if (!k) return; const r = w.relations.find((x) => (x.a === k.a && x.b === k.b) || (x.a === k.b && x.b === k.a))!; r.tradeOpenness = 0.05; r.sciCollaboration = 0.05; r.relation = -0.5; r.tension = 0.6; r.grievances.push('blockade'); }, affinity: (c) => c.traits.strategicAutonomy * 0.7 },
      { id: 'war', label: 'Strike first', consequence: 'War. Grids, ports and homes on both sides; lives; debt; a grievance that outlives everyone alive today.', apply: (w) => { const k = w.pendingConflict.shift(); if (!k) return; startWar(w, k.a, k.b); }, affinity: (c) => c.traits.riskTolerance * 0.4 + c.traits.strategicAutonomy * 0.3 - c.traits.openness * 0.3 },
    ],
  },
  {
    ...
    months: 3,
    options: [
      { id: 'lockdown', label: 'Lock down hard, everywhere', consequence: 'Deaths cut by two thirds; output and investment fall for the duration; trust in institutions decides how long people comply.', apply: (w) => { w.pendingPandemic = false; for (const c of w.civs) c.society.lockdown = 0.8; }, affinity: (c) => c.traits.regulatoryCaution },
      { id: 'targeted', label: 'Protect the old, keep working', consequence: 'Half the deaths avoided, a quarter of the economic cost.', apply: (w) => { w.pendingPandemic = false; for (const c of w.civs) c.society.lockdown = 0.4; }, affinity: () => 0.6 },
      { id: 'open', label: 'Stay open', consequence: 'The economy runs; the pyramid loses its top; trust depends on how bad it gets.', apply: (w) => { w.pendingPandemic = false; for (const c of w.civs) c.society.lockdown = 0; }, affinity: (c) => c.traits.marketOrientation * 0.7 },
    ],
  },
  {
    ...
    months: 12,
    options: [
      { id: 'oversight', label: 'Mandatory verification and human oversight', consequence: 'Adoption resumes slowly and safely; a permanent cost on every deployment; backlash fades.', apply: (w) => { w.pendingAiIncident = false; for (const c of w.civs) { c.society.backlash = Math.max(0, c.society.backlash - 0.1); c.traits.regulatoryCaution = Math.min(1, c.traits.regulatoryCaution + 0.1); } }, affinity: () => 0.7 },
      { id: 'pause', label: 'Pause autonomous deployment for five years', consequence: 'No repeat; five years of lost productivity; neighbours pull ahead.', apply: (w) => { w.pendingAiIncident = false; for (const c of w.civs) { w.techs.ai_agents.adoption[c.id].penetration *= 0.6; c.society.backlash = Math.max(0, c.society.backlash - 0.15); } }, affinity: (c) => c.traits.regulatoryCaution },
  ...
  },
  {
    ...
    options: [
      { id: 'engineer', label: 'Fund sunshade and capture research', consequence: 'Decades of work toward an engineered climate; needs planetary energy to run. Collaboration improves.', params: { collaboration: 1.15, cleanLearningMult: 1.15 }, apply: (w) => { for (const c of w.civs) c.researchCapacity *= 1.05; }, affinity: (c) => c.traits.sciencePriority },
      { id: 'adapt', label: 'Adapt: seawalls, indoor farming, migration', consequence: 'Cheaper now; the temperature keeps rising.', apply: (w) => { for (const c of w.civs) c.food.artificialShare = Math.min(1, c.food.artificialShare + 0.05); }, affinity: (c) => c.traits.regulatoryCaution },
    ],
  },
  {
...
export function stepDecisions(w: WorldState) {
  if (w.tMonths % 3 !== 0) return;
  for (const c of w.civs) {
    for (const d of DECISIONS) {
      const key = `decision-${d.id}-${c.id}`;
      if (w.flags[key]) continue;
      if (w.pendingDecisions.some((p) => p.key === key)) continue;
      if (!d.trigger(w, c)) continue;
      w.flags[key] = true;
  ...
  for (const k of w.pendingConflict) {
    ...
    if (w.pendingDecisions.some((p) => p.defId === 'conflict')) break;
  ...
  for (const p of [...w.pendingDecisions]) if (w.tMonths >= p.deadline) resolveDecision(w, p.key, null);
}

export function resolveDecision(w: WorldState, key: string, optionId: string | null): boolean {
  ...
  if (chosen.effects && chosen.effects.length) w.activeInterventions.push({ id: `dec-${p.defId}`, label: def.title, tStart: w.tMonths, monthsLeft: 120, effects: chosen.effects });
  if (chosen.params) for (const [k, v] of Object.entries(chosen.params)) (w.params as unknown as Record<string, number>)[k] = ((w.params as unknown as Record<string, number>)[k] ?? 1) * (v as number);
  chosen.apply?.(w, c);
