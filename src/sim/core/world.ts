// ─────────────────────────────────────────────────────────────────────────────
// WORLD INITIALIZATION — builds a fully explicit WorldState from defs+params.
// ─────────────────────────────────────────────────────────────────────────────
import type {
  WorldState, CivState, TechState, RelationState, CharacterState, SimParams, CivId, TechId,
} from '../types';
import { CIV_IDS } from '../types';
import { CIV_DEFS } from '../data/civDefs';
import { TECH_DEFS, TECH_IDS } from '../data/techDefs';
import { makeRng } from '../rng';
import { INFLUENCE_START } from '../data/interventions';
import { initialCohorts } from './demography';
import { CALIBRATION } from '../data/calibration';
import { MILESTONES } from '../data/frontier';

export const START_YEAR = 2026;
export const START_MONTH = 8; // September (0-indexed)

function initTech(id: TechId, _params: SimParams): TechState {
  void _params;
  const d = TECH_DEFS[id];
  const adoption = {} as TechState['adoption'];
...
export const CAPACITY_FACTOR = { fossil: 0.55, nuclear: 0.9, solar: 0.22, wind: 0.35, hydro: 0.45, geothermal: 0.88, bioenergy: 0.72, ocean: 0.42, fusion: 0.9 } as const;
const GRID_UTILIZATION = 0.62;

/** Rescale source mix so initial generation ≈ demand × 1.13, grid ≈ peak × 1.12 */
function initEnergy(d: (typeof CIV_DEFS)[number]): CivState['energy'] {
  ...
  let gen = 0;
  for (const k of Object.keys(raw) as Array<keyof typeof raw>) gen += raw[k] * CAPACITY_FACTOR[k] * 8.76;
  const scale = (demand * 1.13) / gen;
  const mk = (gw: number) => ({ cap: gw * scale, underConstruction: 0, queue: [] as Array<{ gw: number; monthsLeft: number }>, condition: 0.94, avgAgeYears: 15, maintenanceBacklog: 0 });
...
function solveOutputScale(d: (typeof CIV_DEFS)[number]): { outputScale: number; outputRefTWh: number } {
...
function initFood(d: (typeof CIV_DEFS)[number]): CivState['food'] {
...
function initCiv(defId: CivId, params: SimParams): CivState {
  const d = CIV_DEFS.find((c) => c.id === defId)!;
  const alloc = {} as Record<TechId, number>;
  ...
  for (const t of TECH_IDS) {
    ...
    let fit = 1;
    if (defId === 'ardan' && (t === 'semiconductor_fab' || t === 'robotics_ind' || t === 'storage_batt')) fit = 1.5;
    if (defId === 'veloria' && (t === 'ai_models' || t === 'ai_agents' || t === 'biotech_med')) fit = 1.5;
    if (defId === 'nemea' && (t === 'solar_pv' || t === 'wind_power' || t === 'grid_transmission')) fit = 1.4;
    const v = pull * fit * (0.5 + d.traits.sciencePriority);
    alloc[t] = v;
    sum += v;
  }
  for (const t of TECH_IDS) alloc[t] /= sum;
...
const CHARACTER_SEEDS: Array<Partial<CharacterState> & { id: string; name: string; civId: CivId; role: string; age: number }> = [
...
export function createWorld(seed: number, params: SimParams, scenarioId: string, scenarioLabel: string): WorldState {
  const techs = {} as Record<TechId, TechState>;
  for (const t of TECH_IDS) techs[t] = initTech(t, params);
  ...
  const characters: CharacterState[] = CHARACTER_SEEDS.map((c) => ({
  ...
  const world: WorldState = {
    ...
    frontier: {
      ...
      energyCaptureTW: 20,
      agi: false, asi: false,
      milestones: MILESTONES.map((m) => ({ id: m.id, status: 'locked' as const, progress: 0, blockers: [] })),
  ...
  for (const c of world.civs) {
    if (techs.robotics_ind.adoption[c.id].penetration > 0.45) world.flags[`robotics-boom-${c.id}`] = true;
    if (techs.transport_ev.adoption[c.id].penetration > 0.5) world.flags[`ev-majority-${c.id}`] = true;
  }
  return world;
}
