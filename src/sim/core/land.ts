// ─────────────────────────────────────────────────────────────────────────────
// LAND — the continent is an island. Every civilization has a fixed amount of
// developable land in 2026, already mostly committed to cities, farms and
// energy. Population growth, sprawl, solar farms and (for a while) farmland
// compete for what is left. When the free share runs out a city has three
// exits, each with a real price:
//   1. build up   — density rises, construction cost per unit rises with it;
//   2. build out  — reclaim shallow shelf from the sea (Singapore, the Dutch
//                   polders), then deeper water at several times the cost;
//                   reclaimed land needs permanent maintenance and is the
//                   first thing sea-level rise takes back;
//   3. build on   — modular floating districts, once the platform technology
//                   is validated; costlier than land-based housing, immune to
//                   sea level, faster to wear.
// Only a solvent, stable, well-run society can afford exits 2 and 3. The
// others live with crowding, informal settlements and emigration.
// ─────────────────────────────────────────────────────────────────────────────
import type { WorldState, CivState, ProjectKind, ProjectState } from '../types';
...
export function seaLevelRiseM(w: WorldState): number {
...
export function housingLandCapexMultiplier(c: CivState): number {
  const L = c.land;
...
export function stepLand(w: WorldState) {
  ...
  for (const c of w.civs) {
    ...
    if (dStock > 0) {
      ...
      L.brownfield -= reuse; L.urban += need - reuse;
    } else if (dStock < 0) {
    ...
    if (L.reclaimed > 0.0005 && L.reclaimedCondition < 0.35) {
      ...
      L.reclaimed -= lost; L.usable -= lost; L.lost += lost;
    ...
    if (rise > 0.25 && protected_ < 0.6) {
      ...
      L.usable = Math.max(0.5, L.usable - lost); L.lost += lost;
    ...
    const maintenance = clamp(
      localMaintainers * 0.45 + c.economy.institutionalCapacity * 0.28 + c.economy.capexAvailability * 0.17 + Math.max(0, c.energy.marginPct) / 100,
      0, 1.15,
    );
    ...
    const conditionStep = (condition: number, annualWear: number, complexity: number, support: number) => {
    ...
    if (L.floating > 0) L.floatingCondition = conditionStep(L.floatingCondition, 0.060, marineSeverity, maintenance);
    else L.floatingCondition = 1;
    if (L.subsea > 0) L.subseaCondition = conditionStep(L.subseaCondition, 0.075, subseaDepthSeverity, maintenance * clamp(c.energy.gridCondition, 0.2, 1));
    else L.subseaCondition = 1;
    if (L.vertical > 0) L.verticalCondition = conditionStep(L.verticalCondition, 0.024, verticalComplexity, maintenance);
    else L.verticalCondition = 1;
    if (L.underground > 0) L.undergroundCondition = conditionStep(L.undergroundCondition, 0.032, undergroundDepth, maintenance * clamp(c.energy.gridCondition, 0.25, 1));
    else L.undergroundCondition = 1;
...
export function floatingPlatformsValidated(w: WorldState): boolean {
  return w.inventions.some((i) => i.id.startsWith('inv-modular-floating-platforms-') && i.status === 'deployed');
}

export function planLandProjects(w: WorldState, c: CivState, propose: ProposeFn) {
  const L = c.land;
  const h = c.housing;
  const def = civDef(c.id);
  ...
  if (!landlocked && L.pressure < 0.85) w.flags[key] = false;
  ...
  if (!landlocked || h.crowding < 0.06) return;
...
function planVerticalProjects(w: WorldState, c: CivState, propose: ProposeFn, landlocked: boolean, needStock: number) {
  const L = c.land;
  const earned = c.frontierReadiness;
  ...
  if (!landlocked && !strategicVertical) return;
  const demandFloor = strategicVertical ? 0.018 : 0.055;
  if (c.housing.crowding < demandFloor && L.pressure < (strategicVertical ? 0.70 : 0.92)) return;
 ...
 */
function planOceanProjects(w: WorldState, c: CivState, propose: ProposeFn) {
  const L = c.land;
  const earned = c.frontierReadiness;
  ...
  if (marine && strategicMarine && c.economy.outputPerCapita > 0.95 && earned > 0.16 && L.floatingCondition > 0.30) {
    const target = clamp(
      0.025 + marinePreference * 0.20 + Math.max(0, L.pressure - 0.72) * 0.38 + Math.max(0, c.housing.crowding - 0.05) * 0.16,
      0.025, 0.38,
    );
    if (L.floating + 0.008 < target) {
      const next = clamp(Math.max(0.016, (target - L.floating) * 0.22), 0.016, 0.055);
      propose(w, c, 'floating_district', next,
        L.floating === 0
  ...
  if (marine && subseaPreference && abundantEnergy && (closedLoop || surfaceServed)
    ...
    if (L.subsea + 0.006 < target) {
      ...
      propose(w, c, 'subsea_habitat', next,
        L.subsea === 0
...
function civPolicyStrength(w: WorldState, civId: CivState['id'], policy: string): number {
  let s = 0;
...
export function landProjectSpec(w: WorldState, c: CivState, kind: ProjectKind, capacity: number): { capex: number; leadMonths: number } | null {
  const L = c.land;
...
export function applyLandCompletion(w: WorldState, c: CivState, p: ProjectState): boolean {
  const L = c.land;
  switch (p.kind) {
    case 'land_reclamation': {
