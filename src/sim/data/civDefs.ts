// ─────────────────────────────────────────────────────────────────────────────
// THREE POLITIES — all modern 2026-level civilizations, differing in
// capability, penetration, institutions and resources (not "era").
// ─────────────────────────────────────────────────────────────────────────────
import type { CivTraits, CivId } from '../types';
...
export interface CivDef {
  ...
  description: string;
  traits: CivTraits;
  population: { total: number; workingShare: number; medianAge: number; education: number; fertility: number; urbanization: number };
  economy: { output: number; unemployment: number; gini: number; publicDebt: number; rdSpendShare: number; capexAvailability: number };
  energy: { demandTWh: number; fossil: number; nuclear: number; solar: number; wind: number; hydro: number; geothermal: number; bioenergy: number; ocean: number; storageGWh: number; gridCapGW: number };
  compute: { accelStock: number; dcCapGW: number };
  society: { trust: number; stability: number; regCaution: number };
  researchCapacity: number;
  // agriculture: land = arable production capacity relative to initial food demand (>1 = potential exporter);
  // climateSensitivity = how hard warming hits yields; artificial0 = initial artificial-food share
  food: { land: number; climateSensitivity: number; artificial0: number };
  // land: the continent is an island. urban/farm/energy = shares of the civ's
  // usable land already committed in 2026; shelf = shallow coastal shelf that
  // can be reclaimed cheaply (as a share of 2026 usable land); seaExposure =
  // share of urban land within reach of sea-level rise.
  land: { urban: number; farm: number; energy: number; shelf: number; seaExposure: number };
  /** 0..1 how strongly the culture holds its own norms (fertility, family) against what trade and media bring in */
...
export function civDef(id: CivId): CivDef {
