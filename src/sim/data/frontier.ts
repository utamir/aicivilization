// ─────────────────────────────────────────────────────────────────────────────
// FRONTIER MILESTONES — the long-horizon ladder from an Earth-bound 2026 to a
// Type I civilization that has stopped being limited by its home planet.
// The calibrated milestone ladder ends inside the home system: no interstellar entries, no orbit
// engineering. Nothing here is scheduled. Every entry
// is either a THRESHOLD (flips when the simulated world state crosses it) or a
// PROGRAM (accumulates progress while its prerequisites hold and while the
// participating civilizations can pay for it). The dates in `illustrativeYear` are
// illustrative horizons used only for narrative ordering. They are NOT forecasts,
// deadlines, or claims that a technology should exist by that date. Conditions, not
// calendar years, unlock milestones.
// ─────────────────────────────────────────────────────────────────────────────
import type { WorldState, CivState } from '../types';

export type MilestoneKind = 'threshold' | 'program';

export interface MilestoneDef {
  id: string;
  name: string;
  era: 1 | 2 | 3;
  illustrativeYear: number;
  kind: MilestoneKind;
  summary: string;
  /** what the simulation actually requires (shown to the player) */
  requires: string[];
  /** returns unmet prerequisite labels; empty = prerequisites satisfied */
  check: (w: WorldState) => string[];
  /** for programs: base years to complete at full funding once prerequisites hold */
  baseYears?: number;
  /** for programs: annual cost as a share of world output */
  costShare?: number;
  /** effect text, applied in core/frontier.ts */
  effect: string;
}

export const ERA_LABELS: Record<1 | 2 | 3, string> = {
  1: 'The Earth-Bound Century',
  2: 'The Interplanetary Era',
  3: 'The Deep-Future Era',
};

const has = (w: WorldState, id: string) => w.frontier.milestones.find((m) => m.id === id)?.status === 'achieved';
const deployed = (w: WorldState, invId: string) => w.inventions.some((i) => i.id.startsWith(`inv-${invId}-`) && i.status === 'deployed');
const anyCiv = (w: WorldState, fn: (c: CivState) => boolean) => w.civs.some(fn);
const worldOutput = (w: WorldState) => w.civs.reduce((a, c) => a + c.economy.output, 0);
...
export const MILESTONES: MilestoneDef[] = [
  // ── Era 1: 2026–2100 ────────────────────────────────────────────────────
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (w.techs.ai_models.cap < 5) out.push(`model capability ${w.techs.ai_models.cap.toFixed(1)}× (<5×)`);
      if (w.techs.ai_agents.cap < 5) out.push(`agent capability ${w.techs.ai_agents.cap.toFixed(1)}× (<5×)`);
      if (Math.min(w.techs.ai_models.reliability, w.techs.ai_agents.reliability) < 0.75) out.push('AI reliability below 75%');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!deployed(w, 'partial-reprogramming')) out.push('partial cellular reprogramming not yet validated');
      if (!anyCiv(w, (c) => w.techs.longevity_bio.adoption[c.id].penetration >= 0.25)) out.push('therapies not yet widely adopted');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!deployed(w, 'modular-floating-platforms')) out.push('floating-platform technology not validated');
      const best = Math.max(...w.civs.map((c) => c.land.reclaimed + c.land.floating * 0.3));
      if (best < 0.10) out.push(`best civilization has added ${(best * 100).toFixed(1)}% of its land (<10%)`);
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!deployed(w, 'fusion-pilot-plant')) out.push('no validated net-electricity pilot plant');
      if (!w.projects.some((p) => p.kind === 'fusion' && p.status === 'operational')) out.push('no fusion plant in service');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!anyCiv(w, (c) => c.space.launchCostIndex <= 0.10)) out.push('launch cost still above 10% of 2026');
      if (!deployed(w, 'closed-loop-habitat')) out.push('closed-loop life support not validated');
      if (!anyCiv(w, (c) => c.space.marsCapacityM >= 0.01)) out.push('no completed Mars colony');
      if (!anyCiv(w, (c) => c.space.marsPopulationM >= 0.01)) out.push('Mars population below 10,000');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!deployed(w, 'in-space-resource-processing')) out.push('in-space resource processing not validated');
      if (w.resources.mineralSpaceInflow < 0.30) out.push(`space inflow ${(w.resources.mineralSpaceInflow * 100).toFixed(0)}% of 2026 demand (<30%)`);
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'fusion_commercial')) out.push('commercial fusion not yet achieved');
      const share = fusionAndSpaceShare(w);
      if (share < 0.5) out.push(`fusion/orbital share ${(share * 100).toFixed(0)}% (<50%)`);
      if (!w.civs.every((c) => c.energy.marginPct >= 20)) out.push('reserve margin below 20% somewhere');
      return out;
    },
  ...
  // ── Era 2: 2100–2300 ────────────────────────────────────────────────────
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!deployed(w, 'orbital-tether-materials')) out.push('tether-grade materials not validated');
      if (!anyCiv(w, (c) => c.space.elevator)) out.push('no elevator completed');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'mars_city')) out.push('no Mars settlement');
      if (!has(w, 'fusion_commercial')) out.push('no commercial fusion');
      if (!has(w, 'agi')) out.push('no general intelligence');
      if (totalOrbitalIndustry(w) < 2) out.push('orbital industry too small');
      return out;
    },
  ...
  // ── Era 3: 2300–2600 ────────────────────────────────────────────────────
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'planetary_energy')) out.push('no planetary energy mastery');
      if (!has(w, 'agi')) out.push('no general intelligence');
      if (totalOrbitalIndustry(w) < 3) out.push('orbital industry too small');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'agi')) out.push('no general intelligence');
      if (w.techs.ai_models.cap < 40) out.push(`model capability ${w.techs.ai_models.cap.toFixed(1)}× (<40×)`);
      if (w.techs.ai_agents.cap < 25) out.push(`agent capability ${w.techs.ai_agents.cap.toFixed(1)}× (<25×)`);
      if (Math.min(w.techs.ai_models.reliability, w.techs.ai_agents.reliability) < 0.9) out.push('AI reliability below 90%');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'asi')) out.push('no superintelligence');
      if (!has(w, 'aging_reversal')) out.push('no aging reversal');
      const cs = w.civs.reduce((a, c) => a + c.compute.supplyFlops, 0);
      if (cs < 60) out.push('compute supply too small');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'asi')) out.push('no superintelligence');
      if (!has(w, 'habitat_network')) out.push('no orbital habitat network');
      if (!has(w, 'space_elevator')) out.push('no space elevator');
      if (totalOrbitalIndustry(w) < 10) out.push('orbital industry too small');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (!has(w, 'climate_control')) out.push('climate not under control');
      if (!anyCiv(w, (c) => c.land.floating > 0.02)) out.push('no floating districts of note');
      if (!anyCiv(w, (c) => c.land.subsea > 0.01)) out.push('nobody lives under the sea');
      if (!anyCiv(w, (c) => c.land.aerial > 0.002)) out.push('nobody lives in the stratosphere');
      if (!has(w, 'habitat_network') && w.civs.reduce((a, c) => a + c.population.offworld, 0) < 1) out.push('orbital population below one million');
      if (!has(w, 'mars_city')) out.push('no Mars settlement');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      ...
      if (!d || (d.status !== 'in_progress' && d.status !== 'achieved')) out.push('Dyson swarm not started');
      if (!has(w, 'habitat_network')) out.push('no orbital habitat network');
      if (!has(w, 'mars_city')) out.push('no Mars settlement');
      return out;
    },
  ...
  },
  {
    ...
    check: (w) => {
      const out: string[] = [];
      if (w.frontier.kardashev < 1.5) out.push(`K = ${w.frontier.kardashev.toFixed(2)} (<1.5)`);
      if (!has(w, 'system_network')) out.push('no system-wide integration');
      return out;
    },
...
export function milestoneDef(id: string): MilestoneDef {
  return MILESTONES.find((m) => m.id === id)!;
}

export function fusionAndSpaceShare(w: WorldState): number {
  let fs = 0, tot = 0;
...
export function totalOrbitalIndustry(w: WorldState): number {
