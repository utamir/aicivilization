// ─────────────────────────────────────────────────────────────────────────────
// DIALOGUE — grounded local conversation. Every answer is assembled from live
// simulation state (the character's civ, domain, mood, history). No remote
// model, no invented facts: numbers come straight from the world.
// ─────────────────────────────────────────────────────────────────────────────
import type { WorldState, CharacterState } from '../sim';
import { taskHorizonHrs, fmtHorizon } from '../sim/core/step';
import { yearOf } from '../sim/core/step';
...
function energyLine(w: WorldState, c: CharacterState): string {
  const civ = w.civs.find((x) => x.id === c.civId)!;
  const m = civ.energy.marginPct;
  if (m > 9) return `Our grid margin sits at ${m.toFixed(0)}% — comfortable. Electricity is at ${civ.energy.priceIndex.toFixed(2)} times the 2026 price.`;
  if (m > 3) return `The grid is tight — only ${m.toFixed(1)}% reserve margin, and prices are ${civ.energy.priceIndex.toFixed(2)}× the 2026 level. Every new data center is a negotiation.`;
...
function aiLine(w: WorldState): string {
...
function laborLine(w: WorldState, c: CharacterState): string {
  const civ = w.civs.find((x) => x.id === c.civId)!;
...
function foodLine(w: WorldState, c: CharacterState): string {
  const civ = w.civs.find((x) => x.id === c.civId)!;
  const f = civ.food;
...
export function characterReply(w: WorldState, c: CharacterState, topic: TopicId): string {
  const civ = w.civs.find((x) => x.id === c.civId)!;
  ...
  switch (topic) {
    case 'work': {
      if (role.includes('energy')) return `It's ${year}, and ${energyLine(w, c)} I'm ${moodWord(c.mood)} about keeping the lights on while demand from AI compute keeps climbing.`;
      if (role.includes('ai') || role.includes('director')) return `${aiLine(w)} My teams feel the acceleration every quarter — tooling that used to assist research now drives it. I'm ${moodWord(c.mood)}.`;
      if (role.includes('fab') || role.includes('semiconductor')) return `Fabrication capacity is the bottleneck everyone discovers eventually. Our fabs run at the edge of what the equipment pipeline allows — capability research means nothing if you cannot manufacture. I'm ${moodWord(c.mood)}.`;
      if (role.includes('labor') || role.includes('union')) return laborLine(w, c);
      if (role.includes('minister')) return `Governing ${civ.name} in ${year} means balancing energy, jobs, and stability — currently at ${(civ.society.stability * 100).toFixed(0)}%. ${energyLine(w, c)}`;
    ...
    case 'worry': {
      const parts: string[] = [];
      if (civ.energy.marginPct < 4) parts.push(`power scarcity (margin ${civ.energy.marginPct.toFixed(1)}%)`);
      if (civ.food.shortage > 0.04) parts.push(`food shortage (${(civ.food.shortage * 100).toFixed(0)}% of demand unmet, prices ${civ.food.priceIndex.toFixed(1)}×)`);
      if (civ.housing.crowding > 0.15) parts.push(`housing crowding (rents ${civ.housing.rentIndex.toFixed(1)}× the 2026 level)`);
      if (civ.waste.accumulation > 0.4) parts.push(`waste piling up (only ${(civ.waste.managedShare * 100).toFixed(0)}% processed)`);
      if (civ.society.backlash > 0.5) parts.push(`automation backlash (${(civ.society.backlash * 100).toFixed(0)}%)`);
      if (civ.economy.unemployment > 0.08) parts.push(`joblessness at ${(civ.economy.unemployment * 100).toFixed(1)}%`);
      if (w.env.warmingC > 1.4) parts.push(`${w.env.warmingC.toFixed(2)}°C of warming`);
