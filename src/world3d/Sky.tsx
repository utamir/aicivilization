...
function sunElevation(mode: 'cycle' | 'day' | 'dusk' | 'night', phase: number): number {
  switch (mode) {
...
export function SkyRig() {
  ...
  const skyUniforms = useMemo(
    () => ({ uNight: { value: 0 }, uDusk: { value: 0 }, uSunDir: { value: new THREE.Vector3(0, 1, 0) } }),
    [],
  );
  ...
  useFrame((_, dt) => {
    ...
    if (mode === 'cycle' && engine.speed !== 0) phase.current = (phase.current + Math.min(dt, 0.1) / DAY_PERIOD) % 1;
    ...
    (window as any).__nightFactor = night;
    (window as any).__sunDir = sunDir;
    ...
    if (stars.current) (stars.current.material as THREE.PointsMaterial).opacity = night * 0.9;
  ...
  return (
    <>
      <fog attach="fog" args={['#b8c8d8', 160, 700]} />
      ...
      <directionalLight
        ...
        shadow-bias={-0.0004}
      />
      <directionalLight ref={moon} position={[-120, 220, -80]} intensity={0} color="#9fb4ff" />
      <mesh scale={[1, 1, 1]} renderOrder={-10}>
        <sphereGeometry args={[780, 24, 16]} />
        <shaderMaterial
          ref={skyMat}
      ...
      <points ref={stars} geometry={starGeo} renderOrder={-9}>
        <pointsMaterial size={1.6} sizeAttenuation={false} color="#cdd8ff" transparent opacity={0} depthWrite={false} />
      </points>
    </>
  );
}
