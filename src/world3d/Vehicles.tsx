...
export function Vehicles({ visualRef }: { visualRef: React.MutableRefObject<WorldVisual | null> }) {
  ...
  useFrame((_, dt) => {
    ...
    (lights.material as THREE.MeshStandardMaterial).emissiveIntensity = night * 2.4;
    ...
    cars.forEach((car, i) => {
      ...
      if (!visible) {
        tmpObj.position.set(0, -80, 0); tmpObj.scale.set(0.001, 0.001, 0.001); tmpObj.rotation.set(0, 0, 0); tmpObj.updateMatrix();
        body.setMatrixAt(i, tmpObj.matrix); lights.setMatrixAt(i, tmpObj.matrix);
        return;
      }
  ...
  return (
    <group>
      <instancedMesh ref={bodyRef} args={[undefined, undefined, cars.length]} frustumCulled={false}>
        <boxGeometry args={[1, 1, 1]} />
        <meshStandardMaterial roughness={0.4} metalness={0.5} />
      </instancedMesh>
      <instancedMesh ref={lightRef} args={[undefined, undefined, cars.length]} frustumCulled={false}>
        <boxGeometry args={[1, 1, 1]} />
        <meshStandardMaterial color="#fff7d6" emissive="#ffedb0" emissiveIntensity={0} />
      </instancedMesh>
    </group>
  );
}
...
// ── Ships ────────────────────────────────────────────────────────────────────
const SHIP_LANES: [number, number][][] = [
  [[-74, -44], [-88, -10], [-84, 40], [-52, 78], [-4, 94]],
  [[-76, -40], [-96, 10], [-70, 70], [-2, 92]],
  [[30, 70], [44, 86], [8, 96], [-2, 92]],
];

export function Ships({ visualRef }: { visualRef: React.MutableRefObject<WorldVisual | null> }) {
  const shipRefs = useRef<THREE.Group[]>([]);
  ...
  return (
    <group>
      {curves.map((_, i) => (
        <group key={i} ref={(g) => { if (g) shipRefs.current[i] = g; }}>
          <mesh castShadow>
            <boxGeometry args={[1.1, 0.5, 3.4]} />
            <meshStandardMaterial color={i === 1 ? '#7a4a3a' : '#3f5a6e'} roughness={0.6} />
          </mesh>
          <mesh position={[0, 0.45, -0.9]}>
            <boxGeometry args={[0.7, 0.5, 0.8]} />
            <meshStandardMaterial color="#d5d9de" roughness={0.7} />
          </mesh>
          <mesh position={[0, 0.42, 0.6]}>
            <boxGeometry args={[0.85, 0.35, 1.4]} />
            <meshStandardMaterial color={i === 2 ? '#4a7a5c' : '#8a6a4a'} roughness={0.7} />
          </mesh>
          {/* running light */}
          <mesh position={[0, 0.85, -0.9]}>
            <sphereGeometry args={[0.09, 6, 6]} />
            <meshStandardMaterial color="#ffe9a8" emissive="#ffd970" emissiveIntensity={1.6} />
          </mesh>
        </group>
      ))}
    </group>
